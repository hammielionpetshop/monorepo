...
export default function LoginPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      // Mode `bo`: identifier boleh email ATAU username. Email bukan lagi syarat — akun yang
      // dibuat lewat Settings > Pengguna wajib punya username tapi emailnya opsional, jadi
      // memaksa format email di sini mengunci mereka dari sistem.
        ...
        <div className="bg-card p-8 rounded-2xl border border-border shadow-xl shadow-black/5">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="space-y-2">
              <label htmlFor="identifier" className="text-xs font-bold text-muted-foreground uppercase tracking-widest ml-1">
                Email atau Username
              </label>
              <input
                id="identifier"
                ...
                placeholder="admin atau admin@hammielion.id"
              />
            </div>
