...
export async function POST(
  ...
  try {
    ...
    const parsed = receivingSchema.safeParse(await req.json());
    if (!parsed.success) {
      return NextResponse.json(
        {
          error:
    ...
    const [po] = await db.select().from(purchaseOrders).where(poWhere).limit(1);
    if (!po) {
      return NextResponse.json(
    ...
    console.error("BO receive PO error:", error);
    return NextResponse.json(
