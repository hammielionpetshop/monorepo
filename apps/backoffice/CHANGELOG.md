...
### Changed
- **Produk yang baru ditambahkan ke keranjang POS muncul di paling atas.** Sebelumnya baris baru selalu masuk ke bawah, jadi kasir harus scroll tiap kali menambah item ketika keranjang sudah panjang. Item yang qty-nya bertambah karena scan ulang produk yang sama tetap di posisinya, tidak ikut naik.
...
### Changed
- **Laporan Nilai Stok FIFO & Ringkasan Stok per Produk tidak lagi merender semua baris sekaligus.** Kedua halaman sekarang dipaginasi (20 baris/halaman) supaya tidak kepanjangan ke bawah untuk katalog besar.
- **Total nilai stok (dan total utang stok di Ringkasan per Produk) dipindah ke bagian atas halaman**, sejajar dengan ringkasan jumlah produk — tidak perlu scroll ke bawah lagi untuk melihat totalnya.
...
### Added
- Kasir bisa menambah, mengubah qty, atau menghapus produk pada PO Internal yang sudah
  dibuat, selama status masih Draft/Menunggu Approval (sebelum diproses cabang pengirim)
  — dari halaman PO Internal di kasir, tombol "+ Tambah Produk" di detail PO.
- Form Buat PO Internal dan form tambah-produk-ke-PO-existing kini menampilkan stok
  cabang tujuan berdampingan dengan stok cabang pengirim yang sudah ada sebelumnya.
...
### Added
- Menu Riwayat Shift: breakdown kasir kini bisa dilihat untuk shift yang masih berlangsung, tanpa menunggu settlement. Angkanya dihitung langsung dari transaksi & pengeluaran berjalan dan ditandai sebagai estimasi.
- Detail shift berstatus "Berlangsung" menampilkan estimasi penjualan, estimasi pengeluaran, dan estimasi kas expected (kas penjualan + pelunasan piutang tunai, di luar modal awal).

### Changed
- Halaman Riwayat Shift kini melebar penuh mengikuti lebar layar (sebelumnya dibatasi), dan modal detail shift diperlebar supaya tabel breakdown 10 kolom tidak perlu digeser.
...
### Fixed
- **Modal master Jagung TT di Toko Depan dibetulkan 305.000 → 255.000 per SAK, menutup rugi yang tinggal menunggu waktu.** Setelah koreksi HPP di `[1.94.2]`, modal master Toko Depan (branch 4) untuk `JAGUNG TT` masih angka lama — GRAM 6.100, KG 12.200, SAK 305.000 — padahal cabang itu menjual SAK 260.000, KG 11.000, GRAM 6.000. Modalnya di atas harga jualnya sendiri di ketiga satuan (−700, −1.400, −45.000). Penjualannya masih untung tipis hanya karena FIFO menutupinya dengan batch 2124 (@5.250, diterima 6 Agu); begitu batch itu habis, `resolveFallbackCostPerBase` mengambil alih dan ruginya berulang persis seperti yang sudah terjadi di Gudang.
  - Modal ditetapkan mengikuti harga kulakan reseller **255.000/SAK** → base 5.100/GRAM, 10.200/KG. Sama persis dengan modal Toko Pusat. Margin Toko Depan kini positif di semua satuan: SAK +5.000, KG +600/+800, GRAM +300/+900.
  - **Yang benar-benar menentukan hanya baris base UOM.** `resolveFallbackCostPerBase` (`stock-service.ts:101-102`) mengembalikan baris base begitu ditemukan dengan `cost_price > 0` — baris KG & SAK tidak pernah dibaca. Karena base Jagung TT adalah GRAM, mengisi modal GRAM dengan harga reseller GRAM Toko Pusat (5.400) justru akan menyiratkan modal SAK 5.400 × 50 = 270.000 dan **mengembalikan rugi −10.000/sak**. Baris KG & SAK tetap diselaraskan agar master data terbaca konsisten.
  - Skrip `apps/db-compare/fix-jagung-tt-modal-toko-depan-20260810.mjs`, lima invarian termasuk penjagaan bahwa tidak ada satuan yang modalnya ≥ harga jualnya dan bahwa Gudang & Toko Pusat tidak ikut berubah. Backup di `petshop.fix_jagung_tt_modal_td_bak_20260810`.
  - **Belum ditangani:** 45 produk lain di Toko Depan yang modal base-nya juga ≥ harga jual terendahnya sendiri — terparah `SPRAY 1L` (modal 215.000 lawan jual 9.000, −206.000), lalu `MAINAN STIK BUNGA` (−5.000), `LOVY` (−4.230), `JAGUNG CRYSTAL SP` (−3.900). Daftar lengkapnya diekspor lewat `apps/db-compare/export-toko-depan-modal-rugi-20260810.mjs`.
  - **Penyebabnya ternyata berbeda dari Jagung TT.** Pendataan menunjukkan modal base Toko Depan **identik dengan Toko Pusat untuk seluruh 936 produk** yang punya keduanya — tidak satu pun berbeda — dan 1.376 baris modal Toko Depan tidak pernah tersentuh sejak impor massal 2026-06-28. Jadi modal 45 produk itu bukan angka basi milik satu cabang, melainkan angka impor yang salah di **kedua** cabang sekaligus. Konsekuensinya, aturan "ikut modal/harga Toko Pusat" tidak bisa memperbaikinya: memakai modal base Toko Pusat menghasilkan margin positif untuk **0** dari 45 produk, dan usulan dari harga reseller Toko Pusat tetap ≥ harga jual untuk 40 di antaranya. Perbaikannya menuntut harga beli riil per produk — masukan bisnis, bukan sesuatu yang bisa diturunkan dari data yang ada.

## [1.94.2] - 2026-08-10

### Fixed
- **Perbaikan data produksi: HPP Jagung TT 1–9 Agustus dinyatakan ulang, laba −1.232.800 → +246.350.** Harga jual `JAGUNG TT` (id 1895) diturunkan sekitar 30 Juli (SAK 310.000 → 260.000, KG 14.000 → 11.000), tapi harga modal di sistem masih memakai angka batch lama: Toko Pusat & Toko Depan 6.100/base (305.000/sak), Gudang 5.900/base (295.000/sak). Modal baru 5.100/base baru masuk lewat batch 2035 di Toko Pusat (5 Agu) dan 5.250 lewat batch 2124 di Toko Depan (6 Agu). Akibatnya 51 item pada 1–9 Agustus membukukan HPP **di atas harga jualnya sendiri** — omzet 8.499.000 lawan HPP 9.731.800.
  - **Gudang tidak pernah menerima batch baru sejak 24 Juli**, sehingga FIFO terus memakai modal 5.900 sampai penjualan terakhir. Puncaknya 9 Agustus: 5 nota, 23 SAK, omzet 5.890.000 lawan HPP 6.785.000 — rugi **−895.000** dalam sehari.
  - HPP dihitung ulang `qty × rasio_ke_base × 5.100` hanya untuk item bermargin minus sejak 1 Agustus WIB. Item bermargin positif, dan seluruh item sebelum 1 Agustus, sengaja tidak disentuh — begitu pula `qty`, `unit_price` & `total_price` yang memang sudah benar.
  - Berbeda dengan kasus LOQY KLG TUNA di `[1.94.1]`, HPP lama di sini **bukan bug data** — 5.900/6.100 memang harga beli riil batch lama. Yang menjadikan koreksi ini sah adalah stok Gudang yang kini **−2.250 base (−45 sak)**: cabang itu menjual barang dari pasokan baru yang lebih murah namun penerimaannya tidak pernah tercatat, sehingga sistem terpaksa memakai modal lama. Stok minus itu sendiri masalah terpisah dan belum ditangani.
- **Modal master Gudang untuk Jagung TT dibetulkan 295.000 → 250.000 per SAK.** Angka lama berada **di atas** harga jual 260.000/sak di cabang yang sama, jadi setiap penjualan Gudang berikutnya dijamin rugi −35.000/sak dan masalahnya akan kambuh esok harinya. Skrip memverifikasi modal master selalu di bawah harga jual di cabang tersebut sebelum commit.
  - Skrip `apps/db-compare/fix-jagung-tt-hpp-20260810.mjs` berjalan dalam satu transaksi dengan tujuh invarian, termasuk penjagaan bahwa item di luar rentang & item bermargin positif tidak ikut masuk backup (bukti tidak tersentuh), dan bahwa tidak ada uom tanpa baris konversi. Backup lossless di `petshop.fix_jagung_tt_bak_20260810`.
  - Tanggal WIB memakai `(created_at AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Jakarta')`, konvensi yang sama dengan `report-service.ts:74`. Menghilangkan `AT TIME ZONE 'UTC'` menggeser hasil **mundur satu hari** untuk transaksi sebelum jam 07:00 UTC.
  - **Belum ditangani:** modal master Toko Depan masih 6.100/12.200/305.000. Penjualannya aman selama batch 2124 (@5.250) belum habis, tapi sesudah itu fallback kembali ke 6.100 dan pola ruginya berulang seperti di Gudang.

## [1.94.1] - 2026-08-10

### Fixed
- **Perbaikan data produksi: satuan LOQY KLG TUNA tertukar, membuat labanya minus 33,8 juta.** Produk `LOQY KLG TUNA` (id 2653) dibuat 2026-07-26 dengan base UOM **DUS**, lalu satuan kecilnya didaftarkan sebagai konversi `PCS ratio 24`. Karena `product_uom_conversions.ratio` selalu dibaca **"1 <uom> = ratio × base"**, baris itu berbunyi "1 PCS = 24 DUS" — persis kebalikan dari yang dimaksud. Dua saudara sekeluarganya (`LOQY KLG CHICKEN`, `LOQY KLG SALMON`) sudah benar sejak awal: base PCS, konversi DUS = 24. Produk 2653 adalah satu-satunya produk di seluruh DB dengan pola terbalik ini.
  - Akibatnya di `transaction-service.ts` (`baseQtyToDeduct = qty × ratio`), setiap penjualan **1 kaleng** memotong stok **24 base** dan membukukan HPP sebesar modal per base (282.480/dus) **× 24 = 6.779.520 per kaleng** yang dijual 14.000. Empat nota di Toko Pusat (5 kaleng, omzet 65.500) membukukan HPP 33.897.600 — laba produk ini terjun ke **−33.832.100** dan ikut menyeret Laporan Laba Rugi.
  - Stok Toko Pusat ikut salah 24×: tercatat **−120**, padahal yang benar **−5** (5 kaleng memang terjual tanpa pernah ada stok masuk di cabang itu — minusnya nyata, hanya besarannya yang keliru).
  - Dibetulkan menjadi base **PCS** dengan konversi **DUS = 24**, `default_cost_price` 11.770 mengikuti dua saudaranya. Stok Gudang dinyatakan ulang dalam base baru: 51 DUS → **1.224 PCS**; batch 1847 ikut dikonversi (sisa 1.224, modal 11.770/PCS).
  - Harga **DUS RETAIL** yang ikut rusak dibetulkan mengikuti LOQY KLG CHICKEN/SALMON — Gudang 583 → **295.000**, Toko Pusat & Toko Depan 14.000 → **310.000**. Angka 583 adalah harga kaleng yang terlanjur dibagi 24, dan 14.000 adalah harga kaleng yang belum dikali sama sekali.
- **HPP historis produk yang sama dihitung ulang.** Sepuluh item transaksi dikoreksi menjadi `qty × rasio_ke_base × 11.770`: 4 item PCS turun dari 33.897.600 → 58.850, dan 6 item DUS di Gudang naik dari **0** → 1.694.880. HPP nol itu penyebab terpisah — batch 1847 lahir dari stock opname 352 saat cabang Gudang belum punya baris `product_uom_costs` untuk produk ini, sehingga `resolveInboundCostPrice` tidak menemukan modal apa pun. Baris modal Gudang (PCS 11.770, DUS 282.480) sekaligus ditambahkan supaya penjualan berikutnya tidak mengulang HPP nol. Laba produk ini kini wajar: DUS +60.120, PCS +6.650.
  - Skrip perbaikan `apps/db-compare/fix-loqy-klg-tuna-uom-20260810.mjs` berjalan dalam satu transaksi dengan enam invarian yang harus lolos sebelum commit (antara lain: tidak ada baris stok yang UOM-nya bukan base UOM produk di **seluruh** DB, sisa batch = terima − terjual, dan tidak ada item ber-HPP melebihi omzet). Backup lossless seluruh baris lama tersimpan di `petshop.fix_loqy_tuna_bak_20260810`.
  - `stock_opname_items` 1977 & 1978 sengaja tidak disentuh — arsip hitungan fisik dibiarkan apa adanya. Begitu pula `qty`, `unit_price` & `total_price` transaksi: rupiah penjualannya sudah benar sejak awal, yang salah hanya HPP-nya.
...
### Fixed
- **Produk yang ada di web POS tidak muncul di Manajemen Harga.** `GET /api/bo/master-data/prices` memakai `JOIN petshop.product_prices` (INNER), sehingga produk yang belum punya satu pun baris harga di cabang terpilih tidak pernah tampil — halaman hanya bisa mengedit harga yang sudah ada, tidak bisa dipakai mengisi harga produk baru. Padahal `GET /api/pos/products` menarik semua produk aktif tanpa syarat harga. Query diubah jadi CTE `pairs`: satuan dasar setiap produk aktif **selalu** jadi satu baris (walau kosong), digabung `UNION` dengan satuan lain yang sudah berharga di cabang itu; agregasi tier dibungkus `COALESCE(... FILTER (WHERE pp.tier_type IS NOT NULL), '{}')` agar baris tanpa harga tidak gagal di `json_object_agg`. Dampak di data produksi (1.089 produk aktif): Toko Pusat 1.440 → 1.555 baris (+115 produk), Toko Depan +111, Gudang +581, dan Toko Raja / Toko Markas / Toko Gudang / HQ dari **halaman kosong** menjadi 1.089 baris siap isi. Urutan baris kini menempatkan satuan dasar di atas satuan konversi dalam satu produk.
- **Harga sudah terisi di Manajemen Harga tapi kartu web POS menulis "No harga".** `product-search-panel.tsx` hanya membaca satu kombinasi persis — `uomId === baseUomId && tierType === 'RETAIL'` — sehingga harga yang tersimpan di satuan konversi (SAK/DUS) atau di tier non-RETAIL diabaikan total. Kini kartu memakai `pickDisplayPrice()` (modul baru `components/pos/price-tier.ts`, dipakai bersama dialog UOM dan bertest unit): prioritaskan satuan dasar, lalu tier menurut urutan RETAIL → GROSIR → MEMBER → RESELLER → DISTRIBUTOR → PROMO, dan menampilkan penanda kecil (`/SAK`, `RESELLER`) bila yang tampil bukan RETAIL di satuan dasar. Label "No harga" kini hanya muncul kalau produk benar-benar tidak punya baris harga sama sekali. Di data produksi label palsu ini hilang untuk 298 produk di Gudang serta 5 produk masing-masing di Toko Pusat & Toko Depan (mis. `KURUNG AYAM BESAR` yang hanya punya tier RESELLER, `HIPRO 781 -2` yang harganya hanya di SAK).
- **Dialog UOM POS terbuka dalam keadaan tanpa harga.** Satuan awal selalu satuan dasar, jadi produk yang harganya hanya ada di satuan konversi langsung memunculkan *"Tidak ada harga untuk UOM ini"* walau harganya ada. Kini dialog memilih satuan pertama yang berharga bila satuan dasar belum berharga, dan daftar tier diurutkan sehingga RETAIL jadi pilihan awal (sebelumnya mengikuti urutan acak dari DB).

### Changed
- **Manajemen Harga kini default ke cabang pengguna**, bukan cabang pertama menurut abjad. Sebelumnya halaman selalu terbuka di cabang berabjad awal (di data produksi: **Gudang**, yang hanya 123 transaksi/30 hari) sementara POS berjalan di Toko Pusat/Toko Depan — pola yang konsisten dengan harga terisi di cabang yang salah tanpa disadari. `page.tsx` membaca `branchId` dari JWT dan meneruskannya sebagai `defaultBranchId`; bila cabang itu tidak ada di daftar, jatuh ke perilaku lama. Selektor cabang juga diberi label "Cabang" dan aksen warna agar cabang aktif tidak terbaca sekilas sebagai filter biasa.
- **Manajemen Harga hanya menampilkan produk aktif** (`p.is_active = true`), sejajar dengan web POS. Di data produksi ini menyembunyikan 1 produk nonaktif (`MR VET DRY TUNA 1 KG T3`) yang harganya tetap tersimpan di DB dan akan tampil lagi bila produk diaktifkan.

### Added
- **`PUT /api/bo/master-data/prices` menolak harga pada satuan non-dasar yang belum punya konversi** (400, mis. *"Satuan DUS pada produk "SPRAY 1L" belum punya konversi ke satuan dasar. Isi kolom Konversi dulu sebelum menyimpan harga."*). Harga seperti ini tidak akan pernah bisa dipilih kasir karena dialog UOM POS hanya menawarkan satuan dasar + satuan yang punya baris konversi. Validasi berlaku untuk `changes` maupun `costChanges`. Alur simpan yang sudah ada tidak terganggu: pembuatan konversi (baris draft "+ satuan" dan pengisian ratio pada baris harga yatim) berjalan lebih dulu sebelum harga dikirim. Di data produksi ada 16 baris warisan seperti ini pada 4 produk (`CANGKIR K2R`, `SPRAY 1L`, `SPRAY 500ML`, `WHISKAS JUNIOR 1,1KG`, semua satuan DUS di Toko Pusat & Toko Depan) — kini barisnya terlihat di Manajemen Harga dan bisa diperbaiki dengan mengisi kolom Konversi; ratio tidak ditebak otomatis karena rasio tersirat dari harga tidak konsisten antar tier.
